...
int recalculateRatioUsingCG (int ploidy, map <string, vector <float> > &read_count,map <string, vector <float> > &GC_profile,
    ...
    for ( it=read_count.begin() ; it != read_count.end(); it++ ) {
        for (int i = 0; i< (*it).second.size(); i++) {
            ...
            float value = (*it).second.at(i);
            float nonN = notNprofile[(*it).first].at(i);
            if ((gc>0)&&(nonN>minMappabilityPerWindow)) {
    ...
    for ( it=read_count.begin() ; it != read_count.end(); it++ ) {
        ...
        for (int i = 0; i< (*it).second.size(); i++) {
            ...
            if ((x>=minGCtoConsider)&&(x<=maxGCtoConsider)&&(nonN>minMappabilityPerWindow)) //update v1.16
...
void calculateBreakpoints(std::map <string, vector <float> > &ratio_profile, std::map <string, vector <int> > &breakpoints) {
