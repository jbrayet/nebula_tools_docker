...
print GENESOUT "Name\tGeneCoord\tDistS\tDistE\tDistTE\tReg\n";
...
sub findClosestTSS {
    ...
    for my $gName (keys %{$genes->{$chro}}) {
	    ...
	    my $geneDist = ($pos-$TSS)*$genes->{$chro}{$gName}{'strand'};
	    $geneDist = ($fpos-$TSS)*$genes->{$chro}{$gName}{'strand'} if (abs($fpos-$TSS)<abs($geneDist)) ;
	    $geneDist = ($lpos-$TSS)*$genes->{$chro}{$gName}{'strand'} if (abs($lpos-$TSS)<abs($geneDist)) ;
	    $geneDist = 0 if ($TSS>=$fpos && $TSS<=$lpos);

	    if (abs($geneDist)<abs($tssDist)) {
		$gene=$gName;
		$tssDist = $geneDist;		
		$teDist = ($pos-$genes->{$chro}{$gName}{'TE'})*$genes->{$chro}{$gName}{'strand'};
	    }	
	    if (abs($geneDist)<$BIGNUMBER) {
###HERE!!!
		print GENESOUT "$gName\t",($fpos-$TSS)*$genes->{$chro}{$gName}{'strand'},"\t",($lpos-$TSS)*$genes->{$chro}{$gName}{'strand'},"\t",($pos-$genes->{$chro}{$gName}{'TE'})*$genes->{$chro}{$gName}{'strand'},"\t",$genes->{$chro}{$gName}{'reg'},"\n";

		if (abs($geneDist)<abs($genes->{$chro}{$gName}{'closestPicDist'})) {
			$genes->{$chro}{$gName}{'closestPicDist'} = $geneDist;
			$genes->{$chro}{$gName}{'closestPicDistTE'} = ($pos-$genes->{$chro}{$gName}{'TE'})*$genes->{$chro}{$gName}{'strand'};
		}
		if ($geneDist>=0 && abs($geneDist)<$genes{$chro}->{$gName}{'closestPositivePicDist'} ) {
			$genes->{$chro}{$gName}{'closestPositivePicDist'} = $geneDist;
		}
		if (($geneDist)*$genes->{$chro}{$gName}{'strand'}<=0 && -1*abs($pos-$TSS)>$genes{$chro}->{$gName}{'closestNegativePicDist'} ) {
