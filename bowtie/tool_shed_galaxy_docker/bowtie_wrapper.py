...
def __main__():
    ...
    bowtie_path='/usr/bin/bowtie/bowtie-1.1.2/'
    ...
    if options.genomeSource == 'history' and not options.do_not_build_index:
        ...
        cmd1 = bowtie_path+'bowtie-build %s -f %s %s' % ( indexing_cmds, ref_file_name, ref_file_name )
        try:
    ...
    try:
        ...
        try:
            ...
            if options.paired == 'paired':
                cmd2 = bowtie_path+'bowtie %s %s -1 %s -2 %s > %s' % ( aligning_cmds, ref_file_name, options.input1, options.input2, options.output )
            else:
                cmd2 = bowtie_path+'bowtie %s %s %s > %s' % ( aligning_cmds, ref_file_name, options.input1, options.output )
            # align
            tmp = tempfile.NamedTemporaryFile( dir=tmp_index_dir ).name
            tmp_stderr = open( tmp, 'wb' )
