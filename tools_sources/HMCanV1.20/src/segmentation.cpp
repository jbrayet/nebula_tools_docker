...
//Changed the name from myFunc to segmentation to show the purpose
//this file has been copied from FREECmyFunc
...
float get_upper_quartile(const std::vector<float>& data_in) {
  int ndatapoints = data_in.size();
  vector <float> data (data_in);
...
// Calculate inter quartile range across individualsmyFunc
// ---------------------------------------------------------------------------
float get_iqr(const std::vector<float>& data_in) {
  int ndatapoints = data_in.size();
  vector <float> data (data_in);
  std::sort (data.begin(), data.end());
  // Get IQR
...
long getLineNumber(std::string const& fileName) {
	string line ;
	long count = 0;
	FILE *stream;
    ...
    if (fileName.substr(fileName.size()-3,3).compare(".gz")==0) {
        ...
        string command = "samtools view "+fileName;
...
int calculateBreakpoints_general(double threshold, int length, vector<float> ratio,vector<int>& bpfinal, int normal_length, int breakPointType) {


	const int maxValue = 3;
	cout<<ratio.size()<<endl;
	...
	for (int i=1; i<=k-1; i++) {
		for (int j=i+1; j<=k; j++) {
			...
			for (int gg=0;gg<j-i;gg++)
				mk[gg] = V[i][i+gg] + J[i+1+gg][j];

			int ind = get_min_index(mk)+1;
			float val = mk[ind-1];
			V[i+1][j] = val;
			jump[i][j] = ind+i;
			mk.clear();
...
float runEM (const vector<float>& x,const vector<float>& y,double * a, int degree, int & maximalNumberOfIterations,int ploidy, int maximalNumberOfCopies, bool intercept) {
	...
	res.clear();
	cout << "Number of EM iterations :" << count << "\n";
	maximalNumberOfIterations=count;
	return rmserror;
}
