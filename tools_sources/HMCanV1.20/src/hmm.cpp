...
void Hmm:: store_prob(unsigned long init, vector<vector<float > >& transition , vector<vector<float > >& emission){
	_initState = init;
	_transition.load(transition, _str2id);
	_emission.load(emission, _str2id);
}
