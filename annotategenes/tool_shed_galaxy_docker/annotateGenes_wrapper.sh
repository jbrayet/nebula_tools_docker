...
echo "Control:" >>$LOG
  ...
  echo "3: cat $R_PATH $DOCKER_PATH/geneDist.R --args $OUTSTAT $OUTPUT $LOG $CONTROLSTAT $PDF $BOOTSTRAP " >>$LOGTMP
  cat $R_PATH $DOCKER_PATH/geneDist.R --args $OUTSTAT $OUTPUT $LOG $CONTROLSTAT $PDF $BOOTSTRAP 2>>$LOGTMP >>$LOGTMP
else
  echo "4:  cat $R_PATH $DOCKER_PATH/geneDist.R --args $OUTSTAT $OUTPUT $LOG $PDF" >>$LOGTMP
  cat $R_PATH $DOCKER_PATH/geneDist.R --args $OUTSTAT $OUTPUT $LOG $PDF 2>>$LOGTMP >>$LOGTMP
fi
